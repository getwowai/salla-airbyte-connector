...
class BaseSallaStream(HttpStream):
    ...
    def request_params(
        ...
        slice_info = f"{stream_slice.get('date_from', '?')} to {stream_slice.get('date_to', '?')}" if stream_slice else "no slice"
        ...
        if stream_slice:
            params[self.date_from_param] = stream_slice.get("date_from", self.start_date)
            params[self.date_to_param] = stream_slice.get("date_to", datetime.now().strftime("%Y-%m-%d"))
    ...
    def stream_slices(
        ...
        if sync_mode == SyncMode.incremental and stream_state:
            ...
            if cursor_value:
                try:
                    start = datetime.strptime(cursor_value[:10], "%Y-%m-%d") - timedelta(days=1)
